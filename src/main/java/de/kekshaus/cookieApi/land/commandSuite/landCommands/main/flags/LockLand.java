...
public class LockLand implements ILandCmd {

	private Landplugin plugin;

	public LockLand(Landplugin plugin) {
		this.plugin = plugin;
	}
	...
	public boolean runCmd(final CommandSender sender, String[] args) {
		...
		if (args.length < 2) {
			...
			if (plugin.getLandManager().switchLockPacket(regionData, loc.getWorld(), true, false)) {
				sender.sendMessage(plugin.getLanguageManager().flagSwitchSuccess
						.replace("{flag}", plugin.getLandManager().getLockName()).replace("{value}", "AKTIV"));
			}
			return true;
		} else if (args[1].toString().equalsIgnoreCase("off")) {
			if (plugin.getLandManager().switchLockPacket(regionData, loc.getWorld(), false, false)) {
				sender.sendMessage(plugin.getLanguageManager().flagSwitchSuccess
						.replace("{flag}", plugin.getLandManager().getLockName()).replace("{value}", "INAKTIV"));
			}
			return true;
		}
		/* Switch flag-state to the other value */
		if (plugin.getLandManager().switchLockPacket(regionData, loc.getWorld(), false, true)) {
			String status = "INAKTIV";
			if (plugin.getLandManager().getLockState(regionData)) {
				status = "AKTIV";
			}
			sender.sendMessage(plugin.getLanguageManager().flagSwitchSuccess
					.replace("{flag}", plugin.getLandManager().getLockName()).replace("{value}", status));
		}
		return true;
	}
