...
public class TNTLand implements ILandCmd {

	private Landplugin plugin;

	public TNTLand(Landplugin plugin) {
		this.plugin = plugin;
	}
	...
	public boolean runCmd(final CommandSender sender, String[] args) {
		...
		/* Permission Check */
		if (!player.hasPermission(plugin.getPermNodes().tntLand)) {
		...
		if (args.length < 2) {
			...
			if (plugin.getLandManager().switchTNTPacket(regionData, loc.getWorld(), true, false)) {
				sender.sendMessage(plugin.getLanguageManager().flagSwitchSuccess
						.replace("{flag}", plugin.getLandManager().getTNTName()).replace("{value}", "AKTIV"));
			}
			return true;
		} else if (args[1].toString().equalsIgnoreCase("off")) {
			if (plugin.getLandManager().switchTNTPacket(regionData, loc.getWorld(), false, false)) {
				sender.sendMessage(plugin.getLanguageManager().flagSwitchSuccess
						.replace("{flag}", plugin.getLandManager().getTNTName()).replace("{value}", "INAKTIV"));
			}
			return true;
		}
		/* Switch flag-state to the other value */
		if (plugin.getLandManager().switchTNTPacket(regionData, loc.getWorld(), false, true)) {
			String status = "INAKTIV";
			if (plugin.getLandManager().getTNTState(regionData)) {
				status = "AKTIV";
			}
			sender.sendMessage(plugin.getLanguageManager().flagSwitchSuccess
					.replace("{flag}", plugin.getLandManager().getTNTName()).replace("{value}", status));
		}
		return true;
	}
